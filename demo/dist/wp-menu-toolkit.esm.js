function animate(options) {
    ...
    element.addEventListener('animationend', function endAnimation() {
...
function menuFromHTML(element, options) {
    ...
    if (!(element instanceof HTMLElement)) {
        throw new Error('Menu element msut be an valid HTMLElement');
    }
    const defaults = {
        action: 'click',
    ...
    settings = Object.assign(Object.assign({}, defaults), options);
    $ulRoot = element;
    $subMenuAnchors = Array.from($ulRoot.querySelectorAll(settings.subMenuAnchorSelector));
    $subMenuListItems = Array.from($ulRoot.querySelectorAll(settings.subMenuListItemSelector));
    /**
     * Create the menu.
     */
    const create = () => {
        ...
        $subMenuAnchors.forEach((a, index) => {
            ...
            if (shouldWrapAnchorToButton(a, level)) {
                button.innerHTML = `${a.textContent}${buttonIcon}`;
    ...
    const handleCloseSubMenu = (event) => {
        var _a, _b, _c;
        const target = event.target;
        const closestSubButtonToChild = (_b = (_a = target.closest('[data-menu="sub-menu"]')) === null || _a === void 0 ? void 0 : _a.parentElement) === null || _b === void 0 ? void 0 : _b.querySelector('[data-menu="sub-toggle"]');
        const closestSubButtonToParent = target.closest('[data-menu="sub-toggle"]');
        const closestSubButton = target.getAttribute('data-menu') === 'sub-toggle' ? target : (closestSubButtonToChild ? closestSubButtonToChild : closestSubButtonToParent);
        const closestListItem = closestSubButton ? closestSubButton.closest('li') : target.closest('li');
        if (!closestListItem) {
            return;
        }
        // Focusable elements.
        const focusableElements = getFocusableElements(closestListItem);
        ...
        if (ESC_KEY === event.key) {
            const closestParentListItem = (_c = target.closest('[data-menu="sub-menu"]')) === null || _c === void 0 ? void 0 : _c.closest('li');
    ...
    const shouldWrapAnchorToButton = (a, level) => {
        ...
        if (typeof settings.shouldWrapAnchorToButton === 'boolean') {
            ...
            should = settings.shouldWrapAnchorToButton(should, a, level, $ulRoot);
        }
        return should;
    };
    /**
    ...
    const getButtonClass = (ul, level) => {
        return typeof settings.buttonClass === 'function' ? settings.buttonClass($ulRoot, ul, level) : settings.buttonClass;
    };
    /**
     * Get button icon.
     */
    const getButtonIcon = (ul, level) => {
        return typeof settings.buttonIcon === 'function' ? settings.buttonIcon($ulRoot, ul, level) : settings.buttonIcon;
    };
    create();
    return {
        openSubMenu,
